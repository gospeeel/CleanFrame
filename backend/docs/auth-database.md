...
`email_verification_codes` хранит одноразовые коды подтверждения для смены email. В БД сохраняется только hash кода, сам код отправляется пользователю по SMTP.
