...
def looks_like_action_after_dialogue(line: str) -> bool:
    ...
    action_markers = {
        "выходит", "входит", "идет", "идёт", "слышит", "видит", "берет", "берёт",
        "хватает", "падает", "ударяет", "бьет", "бьёт", "открывает", "закрывает",
        "ставит", "убирает", "поднимает", "смотрит", "молчит", "кричит",
        "завязывается", "начинается", "камера", "свет", "дверь"
    }
...
def extract_text_from_txt(path: str) -> list[str]:
    """Возвращает список строк из .txt с устойчивым чтением русских кодировок."""
    ...
    raise ValueError(f"Не удалось прочитать .txt файл: {last_error}")


def ensure_scene_header(lines: list[str]) -> list[str]:
    if not lines:
        return lines

    if any(is_scene_header(line) for line in lines):
        return lines

    return ["1. ТЕКСТОВЫЙ СЦЕНАРИЙ"] + lines
...
def parse_script(file_path: str) -> dict:
    """Основная функция: принимает путь к .docx, .pdf или .txt → возвращает JSON."""
    ...
    if file_path.suffix.lower() == '.docx':
        ...
        lines = ensure_scene_header(extract_text_from_txt(str(file_path)))
        return parse_script_lines(lines)
    else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        input_file = input("Введите путь к файлу сценария (.docx, .pdf или .txt): ").strip('"').strip("'")
